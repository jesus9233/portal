...
try{
    ...
    $pdo->run('CREATE TABLE `patient_answers` ( `id` INT NOT NULL AUTO_INCREMENT, `medicalrecord` text,`question_id` INT, `answer` VARCHAR(50), `points` INT, PRIMARY KEY (`id`) )');
}

try{
    $existChk       = $pdo->getResult('SELECT 1 FROM users LIMIT 1');
} catch(Exception $e){
    $pdo->run('CREATE TABLE `users` ( `id` INT NOT NULL AUTO_INCREMENT, `firstname` VARCHAR(30) NOT NULL, `lastname` VARCHAR(30) NOT NULL, `username` VARCHAR(30) NOT NULL, `email` VARCHAR(50), `password` text NOT NULL, `hospital` VARCHAR(30) NOT NULL, PRIMARY KEY (`id`) )');
}
